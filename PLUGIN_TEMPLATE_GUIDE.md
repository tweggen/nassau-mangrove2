...
build a cross-platform audio plugin *in the same way this repository does*, but with a
**different DSP core** ("a different DSP heart"). It captures the technology stack, the
...
*Reference implementation: this repository (Mangrove compressor). Start from
`Source/DSP/compressor_chain.h` (the heart), then `Source/Plugin/MangrovePlugin.cpp`
(the glue), then `Source/Plugin/MangroveUI.cpp` (the UI), then
`Source/Plugin/CMakeLists.txt` (the build).*
